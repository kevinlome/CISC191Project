...
public class WordBattleView
{
	...
	public static void main(String[] args)
	{
		...
		Image scaled = icon.getImage().getScaledInstance(64, 64,
				Image.SCALE_SMOOTH);
		icon = new ImageIcon(scaled);

		int result = JOptionPane.showConfirmDialog(null, panel, "Word Battle",
				JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, icon);

		if (result == JOptionPane.OK_OPTION)
		{
			String p1Name = field1.getText().isBlank() ? "Player 1"
					: field1.getText();
			String p2Name = field2.getText().isBlank() ? "Player 2"
					: field2.getText();
		...
		try
		{
			// Method 1: Try loading from file system first (when running from
			// project root)
			...
			else
			{
				...
				java.net.URL logoURL = WordBattleView.class
						.getResource("/WordBattleLogo.png");
				if (logoURL != null)
				{
		...
		if (logoIcon != null && logoIcon.getIconWidth() > 0)
		{
			...
			Image scaledLogo = logoIcon.getImage().getScaledInstance(125, 125,
					Image.SCALE_SMOOTH);
		...
		for (int row = 0; row < ROWS; row++)
		{
			...
			for (int col = 0; col < COLS; col++)
			{
				...
				textField.addMouseListener(new java.awt.event.MouseAdapter()
				{
					...
					{
						e.consume(); // Consume the event to prevent focus
										// change
					}
				});

				// Add document filter to restrict to single letters only
				AbstractDocument doc = (AbstractDocument) textField
						.getDocument();
				...
				textField.addFocusListener(new java.awt.event.FocusAdapter()
				{
					...
					{
						// Only allow focus if this is the current row and not
						// completed, and it's Player 1's turn
						if (model.isPlayer1Turn() && gridRow == currentRow1
								&& !completedRows1[gridRow])
						{
							keyboard.setCurrentGrid(boxes, gridRow);
							focusRedirecting1[gridRow] = false;
						}
						else if (model.isPlayer1Turn()
								&& !focusRedirecting1[gridRow])
						{
							// Move focus back to the current row for Player 1
							// (only once to prevent loop)
				...
				// Add document listener to handle grid progression within the
				// same row
				textField.getDocument()
						.addDocumentListener(new DocumentListener()
						{
							...
							public void insertUpdate(DocumentEvent e)
							{
								if (textField.getText().length() == 1)
								{
									SwingUtilities.invokeLater(() -> {
										// Only move to next cell within the
										// same row
										int nextCol = gridCol + 1;
										if (nextCol < COLS)
										{
											boxes[gridRow][nextCol]
													.requestFocus();
											currentRow1 = gridRow;
											keyboard.setCurrentGrid(boxes,
													gridRow);
										}
									});
								}
							...
							{
								// On backspace/delete, move to previous box if
								// current is empty
								if (textField.getText().isEmpty()
										&& gridCol > 0)
								{
									SwingUtilities.invokeLater(() -> {
										boxes[gridRow][gridCol - 1]
												.requestFocus();
									});
								}
							}
		...
		for (int row = 0; row < ROWS; row++)
		{
			...
			for (int col = 0; col < COLS; col++)
			{
				...
				textField.addFocusListener(new java.awt.event.FocusAdapter()
				{
					...
					{
						// Only allow focus if this is the current row and not
						// completed, and it's Player 2's turn
						if (!model.isPlayer1Turn() && gridRow == currentRow2
								&& !completedRows2[gridRow])
						{
							keyboard.setCurrentGrid(boxes2, gridRow);
							focusRedirecting2[gridRow] = false;
						}
						else if (!model.isPlayer1Turn()
								&& !focusRedirecting2[gridRow])
						{
							// Move focus back to the current row for Player 2
							// (only once to prevent loop)
							...
							public void insertUpdate(DocumentEvent e)
							{
								if (textField.getText().length() == 1)
								{
									SwingUtilities.invokeLater(() -> {
										...
										if (nextCol < COLS)
										{
											boxes2[gridRow][nextCol]
													.requestFocus();
											currentRow2 = gridRow;
											keyboard.setCurrentGrid(boxes2,
													gridRow);
										}
									});
								}
			...
			int currentRow, int row)
	{
		...
		if (model.checkGuess(guess))
		{
			...
			String playerName = model.isPlayer1Turn() ? player1.getName()
					: player2.getName();
			int choice = JOptionPane.showOptionDialog(null,
					playerName + " wins! The word was: " + guess.toUpperCase()
							+ "\n\nPlayer 1 target: "
							+ model.getPlayer1TargetWord().toUpperCase()
							+ "\nPlayer 2 target: "
							+ model.getPlayer2TargetWord().toUpperCase(),
					"Game Over", JOptionPane.YES_NO_OPTION,
					JOptionPane.INFORMATION_MESSAGE, null,
					new String[] { "Play Again", "Close" }, "Play Again");
		...
		// Switch to next player - use the correct grid based on whose turn it
		// is
		JTextField[][] nextGrid = model.isPlayer1Turn() ? boxes : boxes2;

		// Update current row tracking BEFORE setting focus to allow focus
		// listener to pass
		if (nextGrid == boxes)
		{
			currentRow1 = nextRow;
		...
		JOptionPane.showMessageDialog(null,
				(model.isPlayer1Turn() ? player1.getName() : player2.getName())
						+ "'s Turn!");
	...
	private static void restartGame()
	{
		...
		for (int row = 0; row < ROWS; row++)
		{
			...
			{
				// Clear Player 1 grid
				AbstractDocument doc1 = (AbstractDocument) boxes[row][col]
						.getDocument();
				doc1.setDocumentFilter(null); // Remove filter temporarily
				boxes[row][col].setText("");
				doc1.setDocumentFilter(new LetterOnlyFilter()); // Restore
																// filter
				...
				// Clear Player 2 grid
				AbstractDocument doc2 = (AbstractDocument) boxes2[row][col]
						.getDocument();
				doc2.setDocumentFilter(null); // Remove filter temporarily
				boxes2[row][col].setText("");
				doc2.setDocumentFilter(new LetterOnlyFilter()); // Restore
																// filter
			...
			int[] feedback)
	{
		for (int col = 0; col < COLS && col < feedback.length; col++)
		{
			...
			cell.setCaretColor(cellColor); // Hide cursor with same color as
											// background
